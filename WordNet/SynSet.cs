...
public class SynSet
{
    ...
    public static SynSet Instantiate(string definition, PartOfSpeech partOfSpeech)
    {
        ...
        var numWords = int.Parse(
            GetField(definition, 3, out var wordStart),
        ...
        var numRelationString = GetField(
            definition,
            relationCountField,
            out var relationFieldStart
        );
        ...
        var lexicalRelationSynSets =
            new List<(SynSetRelation relation, SynsetId relatedSetId, int sourceWordId, int
                targetWordId)>();
        ...
        for (var relationNum = 0; relationNum < numRelations; ++relationNum)
        {
            ...
            // add lexical relation
                ...
                lexicalRelationSynSets.Add(
                    (relation, relatedSynSetId, sourceWordIndex, targetWordIndex)
                );
        }

        var relationSynSetsLookup = relationSynSets.ToLookup(x => x.relation, x => x.relatedSetId);

        var lexicalSynSetsLookup = lexicalRelationSynSets.ToLookup(
            x => x.relation,
            x => (x.relatedSetId, x.sourceWordId, x.targetWordId)
        );
        ...
        return new SynSet(
            ...
            words,
            gloss,
            lexicographerFileName,
            relationSynSetsLookup,
            lexicalSynSetsLookup
        );
    }
    ...
    public IEnumerable<SynSet> GetRelatedSynSets(
        SynSetRelation relation,
        bool recursive,
        WordNetEngine engine) => GetRelatedSynSets(new[] { relation }, recursive, engine);
    ...
    public IEnumerable<SynSet> GetRelatedSynSets(
        IReadOnlyCollection<SynSetRelation> relations,
        bool recursive,
    ...
    private int GetDepth(
        IReadOnlyCollection<SynSetRelation> relations,
        ref HashSet<SynSet> synsetsEncountered,
        WordNetEngine engine)
    {
        ...
        foreach (var relatedSynset in GetRelatedSynSets(relations, false, engine))
            if (!synsetsEncountered.Contains(relatedSynset))
            {
                ...
                var relatedDepth = relatedSynset.GetDepth(
                    relations,
                    ref synsetsEncountered,
                    engine
                );
