...
namespace WordNet
{
    ...
    public class WordNetSimilarityModel
    {
        ...
        /// </summary>
        public WordNetEngine WordNetEngine => _wordNetEngine;

        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="wordNetEngine">WordNet engine to use</param>
        public WordNetSimilarityModel(WordNetEngine wordNetEngine) => _wordNetEngine = wordNetEngine;
        ...
        public float GetSimilarity(string string1, PartOfSpeech pos1, string string2, PartOfSpeech pos2, Strategy strategy, params SynSetRelation[] relations)
        {
            float similarity = 0;
            ...
            if (strategy == Strategy.WuPalmer1994Average)
            {
                ...
                var numScores = 0;
                foreach (var synset1 in _wordNetEngine.GetSynSets(string1, pos1))
                    foreach (var synset2 in _wordNetEngine.GetSynSets(string2, pos2))
                    {
            ...
            else if (strategy == Strategy.WuPalmer1994Maximum)
            {
                ...
                foreach (var synset1 in _wordNetEngine.GetSynSets(string1, pos1))
                    foreach (var synset2 in _wordNetEngine.GetSynSets(string2, pos2))
                    {
                        var currentSimilarity = GetSimilarity(synset1, synset2, strategy, relations);
            ...
            else if (strategy == Strategy.WuPalmer1994Minimum)
            {
                ...
                foreach (var synset1 in _wordNetEngine.GetSynSets(string1, pos1))
                    ...
                    {
                        var currentSimilarity = GetSimilarity(synset1, synset2, strategy, relations);
                        if (similarity == -1 || currentSimilarity < similarity)
                            similarity = currentSimilarity;
                    }
            ...
            else if (strategy == Strategy.WuPalmer1994MostCommon)
            {
                ...
                var synset1 = _wordNetEngine.GetMostCommonSynSet(string1, pos1);
                var synset2 = _wordNetEngine.GetMostCommonSynSet(string2, pos2);
        ...
        /// <returns>Similarity</returns>
        public float GetSimilarity(SynSet synset1, SynSet synset2, Strategy strategy, params SynSetRelation[] relations)
        {
            if (relations == null)
                relations = Enum.GetValues(typeof(SynSetRelation)).Cast<SynSetRelation>().ToArray();
