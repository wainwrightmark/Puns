...
public sealed class Syllable : IEquatable<Syllable>
{
    ...
    public IEnumerable<Symbol> Onset =>
        Symbols.TakeWhile(x => x.GetSyllableType() != SyllableType.Vowel);

    public Symbol Nucleus => Symbols.First(x => x.GetSyllableType() == SyllableType.Vowel);

    public IEnumerable<Symbol> Coda =>
        Symbols.SkipWhile(x => x.GetSyllableType() != SyllableType.Vowel).Skip(1);
    ...
    public override bool Equals(object? obj) =>
        ReferenceEquals(this, obj) || obj is Syllable other && Equals(other);
