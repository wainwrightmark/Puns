...
{
    public static SyllableType GetSyllableType(this Symbol symbol) =>
        SyllableTypeDictionary.Value[symbol];

    private static readonly IReadOnlyDictionary<string, SyllableType> TextToSyllableTypeDictionary =
        new Dictionary<string, SyllableType>()
        {
    ...
    private static readonly IReadOnlySet<Symbol> StressedVowels = Enum.GetValues<Symbol>()
        .Where(x => x.ToString().Contains('1') || x.ToString().Contains('2'))
        .ToHashSet();
    ...
    {
        if (s.ToString().Length > 1 && TextToSyllableTypeDictionary.TryGetValue(
            s.ToString().Substring(0, 2),
            out var st
        ))
            return st;
