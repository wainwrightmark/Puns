...
namespace Puns.Strategies
{
    ...
    {
        /// <inheritdoc />
        public SharedPrefixPunStrategy(SpellingEngine spellingEngine, IEnumerable<PhoneticsWord> themeWords) : base(spellingEngine, themeWords)
        {
        }
        ...
        public override IEnumerable<PunReplacement> GetPossibleReplacements(PhoneticsWord originalWord)
        {
            if (originalWord.Syllables.Count > 2)
            {
                ...
                foreach (var themeWord in ThemeWordLookup[firstTwoSyllables])
                {
                    ...
                    {
                        yield return new PunReplacement(PunType.SharedPrefix, themeWord.Text, false, themeWord.Text);
                    }
                }
